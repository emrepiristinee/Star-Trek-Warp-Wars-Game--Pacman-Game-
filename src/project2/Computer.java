...
public class Computer {
	...
	void setScore(char eaten, int x, int y, ClassNumber[] numbers, Trap[] traps) {
		int counter = 0;
		switch (eaten) {
		case '1':
			score += 2;
	...
	void move(char[][] map, ClassNumber[] numbers, Trap[] traps) {
		...
		while (freeze == 0 && flag) {
			...
			if (map[x][y + 1] != '#' && map[x][y + 1] != 'C' && map[x][y + 1] != ' ') {
				map[x][y] = ' ';
				this.y++;
				setScore(map[x][y], x, y, numbers, traps);
	...
	public static void randomPlayerLocation(char[][] map) {
		...
		if (map[Player.cursorX][Player.cursorY] != 'C') {
			map[Player.cursorX][Player.cursorY] = ' ';
			cn.getTextWindow().setCursorPosition(Player.cursorX, Player.cursorY);
			System.out.println(" ");
		}
	}
}