...
public class StarTrek {
	...
	static void addValue(char[][] map, Queue inputQueue) {
		...
		boolean flag = true;
		while (flag) {
			int x = rnd.nextInt(53) + 1;
			int y = rnd.nextInt(21) + 1;
			if (map[x][y] == ' ') {
	...
	public static void displayBackPack(Stack S1ForBackPack) {
		...
		if (S1ForBackPack.isEmpty()) {
			...
			Stack tempQueue = new Stack(S1ForBackPack.size());

			int size = S1ForBackPack.size();

			for (int i = 0; i < size; i++) {
				tempQueue.push(S1ForBackPack.pop());
			}

			size = tempQueue.size();

			if (tempQueue.size() == 1) {
				S1ForBackPack.push(tempQueue.pop());
				...
				for (int j = 0; j < size; j++) {
					...
					cn.getTextWindow().setCursorPosition(56, 11 - j);
					orange();
					System.out.println("| " + peek + " |");
				}
			}
